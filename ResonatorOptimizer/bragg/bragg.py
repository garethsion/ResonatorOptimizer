...
class Bragg:
    ...
    def get_abcd(self,cpw,freq,length):
        # gamma = cpw.gamma(freq,tan_d=1e-07)

        # params = cpw.print_cpw_params()
        # w = params.width.values[0]
        # g = params.gap.values[0]

        # tl = ro.TLParameters(width=w,gap=g,
        #         er=self.__er,h=self.__h,t=self.__t,pen_depth=self.__pen_depth)
        # gamma = tl.gamma(freq,tan_d=1e-07)
        gamma = cpw.gamma(freq,tan_d=1e-10)
        Z0 = cpw.impedance_total()
    ...
    def cpw_section(self,width, gap, length):
        cpw = ro.CPW(width=width,gap=gap,length=length,fo=self.__fo, er=self.__er, h=self.__h, t=self.__t, pen_depth=self.__pen_depth)
        return cpw
    ...
    def bragg_reflector_transmission(self,freq,lowZ, highZ): 
        lowZ_abcd = self.get_abcd(lowZ,freq,lowZ.print_cpw_params().length.values[0])
        highZ_abcd = self.get_abcd(highZ,freq,highZ.print_cpw_params().length.values[0])
            
        mirror_lhs = [np.matmul(lowZ_abcd[i], highZ_abcd[i]) for i in range(len(freq))]
        # mirror_rhs = [np.matmul(highZ_abcd[i], lowZ_abcd[i]) for i in range(len(freq))]
        # bragg = [np.matmul(mirror_lhs[i]**self.__no_mirrors, 
        #     mirror_lhs[i]**self.__no_mirrors) for i in range(len(freq))]
        bragg = [mirror_lhs[i]**self.__no_mirrors for i in range(len(freq))]

        sp = ro.Sparams(freq=freq)
        bragg_s21 = sp.s21_from_abcd(bragg)
        return bragg_s21

    def bragg_reflector_reflection(self,freq,lowZ, highZ): 
        lowZ_abcd = self.get_abcd(lowZ,freq,lowZ.print_cpw_params().length.values[0])
        highZ_abcd = self.get_abcd(highZ,freq,highZ.print_cpw_params().length.values[0])
        ...
        sp = ro.Sparams(freq=freq)
        bragg_s11 = sp.s11_from_abcd(bragg)
        return bragg_s11
